...
export async function getDashboardData() {
  ...
  const totalSamples = await db.sample.count({ where: { labId, deletedAt: null } })
  const pendingSamples = await db.sample.count({ where: { labId, deletedAt: null, status: { in: ["pending", "registered"] } } })
  const inProgressSamples = await db.sample.count({ where: { labId, deletedAt: null, status: { in: ["assigned", "testing"] } } })
  const completedSamples = await db.sample.count({ where: { labId, deletedAt: null, status: { in: ["completed", "reported"] } } })
  const totalReports = await db.report.count({ where: { labId } })
  const totalInvoices = await db.invoice.count({ where: { labId, deletedAt: null } })
  ...
  // Financial
  const invoices = await db.invoice.findMany({ where: { labId, deletedAt: null } })
  ...
  if (roleName === "Chemist") {
    myAssigned = await db.sample.count({ where: { labId, deletedAt: null, assignedToId: user.id, status: { in: ["assigned", "testing"] } } })
  ...
  if (roleName === "Sampler") {
    ...
    todayCollections = await db.sample.count({
      where: { labId, deletedAt: null, collectedById: user.id, collectionDate: { gte: today } }
    })
  }
